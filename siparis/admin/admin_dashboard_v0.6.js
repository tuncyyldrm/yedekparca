    ...
    fetch(`order_details.php?id=${orderId}`)
        ...
        .then(data => {
            if (data.order) {
                ...
                itemsContainer.innerHTML = data.items.map((item, index) => `
                    <div class="item-row" data-id="${item.id}" data-index="${index}">
                        ...
                        <button type="button" class="delete-item-button">Sil</button>
                    </div>`).join('');
                
                // Ürün silme özelliği
                itemsContainer.querySelectorAll('.delete-item-button').forEach(button => {
                    button.addEventListener('click', function() {
                        const itemRow = this.closest('.item-row');
                        itemRow.dataset.deleted = 'true'; // Silinen ürün olarak işaretle
                        itemRow.remove();
                        updateTotalPrice();
                    });
                });
                ...
                document.getElementById('add-item-form').style.display = 'block'; // Ürün ekleme formunu göster
            }
        })
...
document.getElementById('add-item-button').addEventListener('click', function() {
    ...
    if (itemName && itemCount > 0 && itemPrice > 0) {
        const itemsContainer = document.getElementById('edit-items-container');
        const newItemRow = document.createElement('div');
        newItemRow.className = 'item-row';
        newItemRow.innerHTML = `
            <input type="text" value="${itemName}" class="item-name" readonly />
            <input type="number" value="${itemCount}" class="item-count" />
            <input type="number" value="${itemPrice}" class="item-price" />
            <button type="button" class="delete-item-button">Sil</button>
        `;
        itemsContainer.appendChild(newItemRow);
        
        // Yeni ürün silme özelliği ekle
        newItemRow.querySelector('.delete-item-button').addEventListener('click', function() {
            const itemRow = this.closest('.item-row');
            itemRow.dataset.deleted = 'true'; // Silinen ürün olarak işaretle
            itemRow.remove();
            updateTotalPrice();
        ...
        alert('Lütfen tüm ürün bilgilerini doğru şekilde doldurduğunuzdan emin olun.');
    }
});

// Ürün eklemeyi iptal etme fonksiyonu
document.getElementById('cancel-add-item-button').addEventListener('click', function() {
    document.getElementById('item-name').value = '';
    document.getElementById('item-count').value = '';
    document.getElementById('item-price').value = '';
    document.getElementById('add-item-form').style.display = 'none'; // Formu gizle
});
...
window.saveOrderChanges = function() {
    ...
    const items = Array.from(document.querySelectorAll('.item-row')).map(row => ({
        id: row.dataset.id, // Ürün ID'si
        item_name: row.querySelector('.item-name').value,
        item_count: row.querySelector('.item-count').value,
        item_price: row.querySelector('.item-price').value,
        deleted: row.dataset.deleted === 'true' // Silinip silinmediğini kontrol et
    }));
    ...
    .then(data => {
        if (data.success) {
            ...
            fetchOrders(); // Sipariş listesini yeniden yükleyin
